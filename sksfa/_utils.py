"""
This file contains some utility functions to use with SFA.
"""
import numpy as np

def randomWalkFromCSC(csc_matrix, n_steps=10, starting_node=None, restart_rate=0):
    """
    This method produces an index trajectory on a graph by random walk, when the
    adjacency matrix is given in Compressed Sparse Column (CSC) Matrix format as
    implemented by scipy.
    The trajectory can be used to emulate spectral embeddings with SFA.

    Parameters
    ----------
    csc_matrix : sparse matrix (csc) of shape (n_nodes, n_nodes)
